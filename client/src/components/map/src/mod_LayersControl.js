...
export class ControlledLayer extends Component<ControlledLayerProps> {
  ...
  componentDidUpdate({ checked }: ControlledLayerProps) {
    ...
    if (this.props.checked === true && (checked == null || checked === false)) {
      ...
      //Voisko tähän tehdä vaan piilotuksen?
...
class BaseLayer extends ControlledLayer {
  ...
  addLayer = (layer: Layer) => {
    ...
    const { addBaseLayer, checked, name, elementclass } = this.props
    addBaseLayer(layer, name, checked, elementclass)
  }
}
...
class Overlay extends ControlledLayer {
  ...
  addLayer = (layer: Layer) => {
    this.layer = layer // Keep layer reference to handle dynamic changes of props
    const { addOverlay, checked, name, elementclass } = this.props
    const elementclass = 'luokka'
    addOverlay(layer, name, checked,elementclass)
  }
}
...
class LayersControl extends MapControl<LeafletElement, LayersControlProps> {
  ...
  addBaseLayer(layer: Layer, name: string, checked: boolean = false, elementclass: string) {
    if (checked && this.props.leaflet.map != null) {
      this.props.leaflet.map.addLayer(layer, elementclass)
    }
    this.leafletElement.addBaseLayer(layer, name, elementclass)
  }

  addOverlay(layer: Layer, name: string, checked: boolean = false, elementclass = string) {
    if (checked && this.props.leaflet.map != null) {
      this.props.leaflet.map.addLayer(layer)
    }
    this.leafletElement.addOverlay(layer, name, elementclass)
  }
  ...
  render() {
    ...
    console.log('LayersControl : children: ',children)
